...
class User extends Main{
	...
	public function  __construct()	{
		...
		$search = "SELECT * from trust WHERE 
		trusted_by_id='$trusted_by'AND user_id='$user_id'    ";	
		if($this->db->exists($search))	return true;
		else  return false;
		...
		public function addUser($data)
		{
			...
			$q = "INSERT INTO `users` (`user_id`, `date`, `password`, `first_name`, `last_name`, `email`, `mobile`) 
		...
		public function checkAlreadyBid($user_id , $post_id)
		{
			...
			$s = "SELECT * from 
			listings where post_id = '$post_id' AND user_id = '$user_id'";
		...
		public function getTotalListings($user_id)
		{
			...
			$s = "SELECT count(*) as total from posts where user_id = '$user_id' AND active='y'";
		...
		public function AddOngoing($list_id,$logged_user)
		{
			...
			if ($this->db->query($i))
			{
				...
				$u = "UPDATE posts SET active = 'n' WHERE id ='$listings[post_id]'  "	;
				$this->db->query($u);
