		...
		public function show_bar($type,$msg,$ret=false)
		{
			 ...
			 public function adminLogin($data)	
 		 {
			  ...
			  $pass   =  md5(mysql_real_escape_string($data['user_pass']));
				...
				$q  = "SELECT username , password from  admin where 
				username ='$username'
				AND  password = '$pass'";
			...
			public function getUpdateUsers($data, $files, $id)
		{
				$message="";
					...
					if($message=="")
					{
						 ...
						 $mobile	=$data["mobile"];
						 $bank		=$data["bank"];
	         $stt = "UPDATE users SET first_name='$fname', last_name='$lname',password='$pwd',email='$email',mobile='$mobile',bank='$bank', pic='$pic' WHERE user_id='$id'";
			 $result=$this->db->query($stt);
