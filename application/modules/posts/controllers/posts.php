...
class Posts extends Auth_Controller
{
	...
	public function manage($offset = '')
	{
		...
		$order_by  = 'id asc';
		$query = $this->post->get_with_limit($limit, $offset, $order_by);
	...
	public function add()
	{
		 ...
		 * ----------------------------------------------------------------------
		 */
		$this->form_validation->set_rules('post_title', 'Post Title', 'trim|required|min_length[4]');
		...
		else
		{
			...
			if (isset($submit['add']))
			{
				...
				$data = array(
					...
					'post_slug'       => url_title($this->input->post('post_title', TRUE), 'underscore', TRUE),
					...
					'post_amount'     => $this->input->post('post_amount', TRUE),
					'post_currency'   => $this->input->post('post_currency', TRUE),
					'post_pic'        => $this->input->post('post_pic', TRUE),
					'post_created_at' => set_now(),
					'post_updated_at' => set_now(),
					'post_active'     => $this->input->post('post_active', TRUE),
				);

				// Insert the new page database record.
				$insert_id = $this->post->_insert($data);

				$data2['msg'] = "The post has now been created.";

				redirect('posts/manage', 'refresh');
			}
		}
	}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			$query = $this->post->get_where(array('id' => $id));
			$row   = $query->row();

			// Setup the view with the database record data.
			$data['post_title']      = $row->post_title;
			$data['post_slug']       = $row->post_slug;
			$data['post_user_id']    = $row->post_user_id;
			$data['post_content']    = $row->post_content;
			$data['post_type']       = $row->post_type;
			$data['post_amount']     = $row->post_amount;
			$data['post_currency']   = $row->post_currency;
			$data['post_pic']        = $row->post_pic;
			$data['post_active']     = $row->post_active;

			$data['page_title'] = 'Edit Post';
			$data['module']     = 'posts';
			$data['view_file']  = "post_edit";
		...
		else
		{
			...
			if (isset($submit['update']))
			{
				...
				$result = $this->post->_update(array('id' => $id), $data);

				$data2['msg'] = "The post has now been edited.";
