...
class Log extends Auth_Controller
{
	...
	public function manage($offset = '')
	{
		...
		$order_by  = 'id asc';
		$query = $this->log->get_with_limit($limit, $offset, $order_by);
	...
	public function add()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			$data['page_title'] = 'Add Log';
			$data['module']     = 'logs';
			$data['view_file']  = "log_add";
		...
		else
		{
			...
			if (isset($submit['add']))
			{
				...
				$insert_id = $this->log->_insert($data);

				$data2['msg'] = "The Log has now been created.";

				// Redirect back to the manage view.
				redirect('logs/manage', 'refresh');
			}
		}
	}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			// Setup the view and display it.
			$data['page_title'] = 'Edit Log';
			$data['module']     = 'logs';
			$data['view_file']  = "log_edit";
		...
		else
		{
			...
			if (isset($submit['update']))
			{
				...
				$result = $this->log->_update(array('id' => $id), $data);

				$data2['msg'] = "The Log has now been edited.";
