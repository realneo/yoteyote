...
class IWill extends Auth_Controller
{
	...
	public function manage($offset = '')
	{
		 ...
		 */
		$config = array(
			'base_url'      => base_url('iwill/manage/'),
			'uri_segment'   => 3,
	...
	public function add()
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			$data['page_title'] = 'Add I Will';
			$data['module']     = 'iwill';
			$data['view_file']  = "iwill_add";
		...
		else
		{
			...
			if (isset($submit['add']))
			{
				...
				$insert_id = $this->iwill->_insert($data);

				$data2['msg'] = "The I Will has now been created.";

				// Redirect back to the manage view.
				redirect('iwill/manage', 'refresh');
			}
		}
	}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			$query = $this->iwill->get_where(array('id' => $id));
			$row   = $query->row();
			...
			// Setup the view and display it.
			$data['page_title'] = 'Edit I Will';
			$data['module']     = 'iwill';
			$data['view_file']  = "iwill_edit";
		...
		else
		{
			...
			if (isset($submit['update']))
			{
				...
				$result = $this->iwill->_update(array('id' => $id), $data);

				$data2['msg'] = "The I Will has now been edited.";
	...
	{
		$this->iwill->_delete(array('id' => $id));
