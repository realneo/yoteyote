...
class Widget extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('mdl_widget', 'widget');
	}
	...
	public function manage($offset = '')
	{
		 ...
		 */
		$config = array(
			'base_url'      => base_url('widgets/manage/'),
			'uri_segment'   => 3,
		...
		$data['page_title']  = 'Manage Widgets';
	...
	public function add()
	{
		...
		else
		{
			...
			if (isset($submit['add']))
			{
				...
				$insert_id = $this->widget->_insert($data);

				$data2['msg'] = "The Widget has now been created.";

				// Redirect back to the manage view.
				redirect('widgets/manage', 'refresh');
			}
		}
	}
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run($this) == FALSE)
		{
			...
			$query = $this->widget->get_where(array('id' => $id));
			$row   = $query->row();
		...
		else
		{
			...
			if (isset($submit['update']))
			{
				...
				$result = $this->widget->_update(array('id' => $id), $data);

				$data2['msg'] = "The Widget has now been edited.";
	...
	{
		$this->widget->_delete(array('id' => $id));
